        ...
        if (typeof val === "number") {
          ...
          commit = (v) => {
            images.value = images.value.map((i) =>
              i.id === selImgId ? { ...i, [field]: v } as ImagePlane : i
            );
          };
        }
      }
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
        ...
        if (r.ray.intersectPlane(imgDrag.plane, hit)) {
          images.value = images.value.map((i) =>
            i.id === imgDrag.id
              ? {
                ...i,
                x: parseFloat((hit.x + imgDrag.offsetX).toFixed(3)),
                z: parseFloat((hit.z + imgDrag.offsetZ).toFixed(3)),
              }
              : i
          );
        }
      ...
      if (drag) {
        ...
        drag.commit(
          parseFloat(
            Math.max(
              drag.min,
              Math.min(drag.max, drag.startValue + dot * drag.sensitivity),
            ).toFixed(3),
          ),
        );
        return;
      ...
      if (gizmoGroup.visible) {
        ...
        setHover(
          hits.length > 0 ? hits[0].object.userData.field as string : null,
        );
      } else {
        setHover(null);
      }
    ...
    for (const img of images.value) {
      if (!imageMeshMapRef.current.has(img.id)) {
        ...
        const texture = texLoader.load(img.url, (tex) => {
          ...
          images.value = images.value.map((i) =>
            i.id === img.id
              ? { ...i, height: parseFloat((i.width / aspect).toFixed(2)) }
              : i
          );
        });
      ...
      <aside style="position:absolute;top:0;left:0;width:214px;height:100%;background:rgba(8,8,18,0.88);color:#ccc;padding:12px;box-sizing:border-box;display:flex;flex-direction:column;gap:8px;overflow-y:auto;">
        ...
        {images.value.map((i) => (
          <div
            key={i.id}
            onClick={() => {
              selectedImgId.value = i.id === selectedImgId.value ? null : i.id;
              if (selectedImgId.value !== null) selected.value = null;
            }}
            style={`padding:5px 8px;border-radius:3px;cursor:pointer;font-size:11px;user-select:none;
              background:${
              ...
              border:1px solid ${
              i.id === selectedImgId.value ? "#2a5533" : "transparent"
            };`}
          >
              ...
              >
                <span style="display:flex;justify-content:space-between;">
                  ...
                  <span style="color:#88ffaa;">
                    {(selectedImg.value![key] as number).toFixed(unit ? 0 : 2)}
                    {unit ?? ""}
                  </span>
                </span>
                <input
